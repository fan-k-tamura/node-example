...
async function addReleaseToNotion() {
  try {
    ...
    const dbId = process.env.NOTION_RELEASE_NOTE_DATABASE_ID || '';
    ...
    if (!process.env.NOTION_INTEGRATION_TOKEN) {
      throw new Error('NOTION_INTEGRATION_TOKEN environment variable is not set');
    }
    ...
    if (releaseBody.trim()) {
      ...
      // Convert markdown release notes to Notion blocks
      const contentBlocks = markdownToBlocks(releaseBody);
      notionBlocks = [...notionBlocks, ...contentBlocks];
    }

    try {
      // Notionデータベース連携
      const databaseId = process.env.NOTION_RELEASE_NOTE_DATABASE_ID;
